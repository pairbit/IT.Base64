...
public class Base64Encoder
{
    ...
    public static readonly Base64Encoder Url = new("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_"u8, true);
    ...
    private readonly bool _isVectorUrl;
    ...
    public Base64Encoder(ReadOnlySpan<byte> bytes) : this(bytes, false) { }

    private Base64Encoder(ReadOnlySpan<byte> bytes, bool isVectorUrl)
    {
    ...
    public EncodingStatus TryEncode128(Guid value, Span<byte> encoded)
    {
        if (encoded.Length < 22) return EncodingStatus.InvalidDestinationLength;

        if (_isVectorUrl)
            VectorBase64Url.Encode128(ref Unsafe.As<Guid, byte>(ref value), ref MemoryMarshal.GetReference(encoded));
        else
            UnsafeBase64.Encode128(ref _bytes[0], ref Unsafe.As<Guid, byte>(ref value), ref MemoryMarshal.GetReference(encoded));

        return EncodingStatus.Done;
    }

    public EncodingStatus TryEncode128(Guid value, Span<char> encoded)
    {
        if (encoded.Length < 22) return EncodingStatus.InvalidDestinationLength;
        ...
        if (_isVectorUrl)
            ...
            UnsafeBase64.Encode128(ref _chars[0], ref Unsafe.As<Guid, byte>(ref value), ref MemoryMarshal.GetReference(encoded));

        return EncodingStatus.Done;
    }

    /// <exception cref="ArgumentOutOfRangeException"/>
    public void Encode128(Guid value, Span<byte> encoded)
    {
        if (encoded.Length < 22) throw new ArgumentOutOfRangeException(nameof(encoded), encoded.Length, "length < 22");
    ...
    /// <exception cref="ArgumentOutOfRangeException"/>
    public void Encode128(Guid value, Span<char> encoded)
    {
        if (encoded.Length < 22) throw new ArgumentOutOfRangeException(nameof(encoded), encoded.Length, "length < 22");
    ...
    public byte[] Encode128ToBytes(Guid value)
    {
        var encoded = new byte[22];

        if (_isVectorUrl)
            VectorBase64Url.Encode128(ref Unsafe.As<Guid, byte>(ref value), ref encoded[0]);
        else
            UnsafeBase64.Encode128(ref _bytes[0], ref Unsafe.As<Guid, byte>(ref value), ref encoded[0]);

        return encoded;
    }

    public char[] Encode128ToChars(Guid value)
    {
        var encoded = new char[22];
        ...
        if (_isVectorUrl)
            ...
            UnsafeBase64.Encode128(ref _chars[0], ref Unsafe.As<Guid, byte>(ref value), ref encoded[0]);

        return encoded;
    }

    public string Encode128ToString(Guid value) => string.Create(22, value, (chars, value) =>
    {
        if (_isVectorUrl)
            VectorBase64Url.Encode128(ref Unsafe.As<Guid, byte>(ref value), ref MemoryMarshal.GetReference(chars));
        else
            UnsafeBase64.Encode128(ref _chars[0], ref Unsafe.As<Guid, byte>(ref value), ref MemoryMarshal.GetReference(chars));
    });
